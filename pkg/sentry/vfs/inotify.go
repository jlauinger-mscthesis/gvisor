...
//
// Lock ordering:
//   Inotify.mu -> Watches.mu -> Watch.mu -> Inotify.evMu
//
// +stateify savable
type Inotify struct {
...
// AddWatch constructs a new inotify watch and adds it linuxto the target. It
...
func (w *Watches) Remove(id uint64) {
	...
	if w.ws == nil {
		...
		// See the comment in Watches.handleDeletion for why this race exists.
		return
	}
...
type Watch struct {
	...
	// releaseOnce is used to release the reference on target upon destruction.
...
// ExcludeUnlinkedChildren indicates whether the parent of the watched object
// should continue to be notified of events after the target has been unlinked.
//
...
// release prepares the watch for destruction by...
// release does not cause any new events to be generated. The caller
...
// InotifyUnlink sends the appriopriate notifications to the watch sets of the
// link being removed and its parent.
func InotifyUnlink(self, parent *Watches, name string) {
