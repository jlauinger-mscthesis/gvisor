...
// A CancelFunc tells a Clock to abandon its work. A CancelFunc does not wait
...
type Clock interface {
	...
	// to cancel the call.
	AfterFunc(l sync.Locker, d time.Duration, f func()) CancelFunc
}
...
func (e *neighborEntry) setStateLocked(next NeighborState) {
	...
	case Failed:
		...
		e.mu.cancel = e.clock.AfterFunc(&e.mu, config.UnreachableTime, func() {
			if ok := e.nic.neigh.removeEntryLocked(e); !ok {
				panic(fmt.Sprintf("Failed to remove neighbor entry; entry for %q not found", e.mu.neigh.Addr))
			}
		})
